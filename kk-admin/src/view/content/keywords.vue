...
import AddOrUpdate from './keywords-add-or-update.vue'
...
export default {
  ...
  data() {
    return {
      mixinViewModuleOptions: {
        ...
        getDataListModel: Keyword.getKeywords,
        deleteDataModel: Keyword.delectKeyword,
      },
      previewContent:'',
  ...
  methods: {
    async getDomainNum(id) {
      ...
      const domains = await Keyword.getCategoryDomains(id)
      const num = domains.length
      this.$alert(`当前分组下共有: ${num} 个域名`, '标题名称', {
    ...
    async preview(row){
      let {path}= row
      const data = await Keyword.getPreview({path})
      this.previewContent = data.content
      this.dialogVisiblePreview = true
    },
    async download(id){
      const data = await Keyword.download(id)
      window.open(data)
    },
    setConfig(id, row) {
      this.currentRow = row
