...
import AddOrUpdate from './sub-add-or-update.vue'
...
export default {
  ...
  data() {
    return {
      mixinViewModuleOptions: {
        ...
        getDataListModel: Sub.getSubs,
        deleteDataModel: Sub.delectSub,
      },
      previewContent:'',
  ...
  methods: {
    async getDomainNum(id) {
      ...
      const domains = await Sub.getCategoryDomains(id)
    ...
    async preview(row){
      let {path}= row
      const data = await Sub.getPreview({path})
      this.previewContent = data.content
      this.dialogVisiblePreview = true
    },
    async download(id){
      const data = await Sub.download(id)
      window.open(data)
    },
    setConfig(id, row) {
      this.currentRow = row
